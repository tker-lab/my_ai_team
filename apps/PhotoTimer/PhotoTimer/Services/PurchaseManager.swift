...
/// 課金状態(購入・復元)を一手に管理するクラス。CEO決定(2026-09-06):有料にする機能は
/// 「演出パターン・自作リスト(リスト作成)・場所での絞り込み」の3つで、個別課金にはせず
...
final class PurchaseManager: ObservableObject {
    ...
    /// 購入済みかどうか。`FeatureFlags.swift`の3つの機能フラグ(演出パターン・自作リスト・
    /// 場所での絞り込み)はすべてここを見て判定する。
    ...
    private init() {
        // 【DEBUG限定・2026-09-06追加】演出パターン・自作リスト・場所での絞り込み自体の動作を
