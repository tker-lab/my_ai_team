...
enum ImageAnalyzer {
    ...
    private static func analyzeMood(cgImage: CGImage) -> MoodTag? {
        ...
        // 【8-9対応・2026-09-05】以前は4つのcaseに分かれていたが、「緑」を削除した名残で
        // 隣り合う2つずつが全く同じ処理(warm/warm、cool/cool)になっていて紛らわしかった
        // (動作自体は元々正しかった)。同じ結果を返す範囲をまとめて2つのcaseに整理した。
        switch h {
        case 0.0..<0.32, 0.94...1.0:
