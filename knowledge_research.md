...
**結論**:**ある。Anthropic公式のChrome拡張機能「Claude in Chrome」+ Claude Codeの`--chrome`起動オプションで実現できる**
- 使っているChromeのログイン状態をそのまま共有する(共有ログイン中のサイトなら追加ログイン不要)。ログイン画面やCAPTCHA(不正ログイン防止の画像認証)に当たった時だけ一時停止してCEOに手動対応を求める、という設計。**「CEOがログイン(2段階認証含む)だけ済ませ、以降の画面操作はAIに任せる」という理想の流れそのもの**
- 対応ブラウザ:Chrome/Edge/Brave/Arc/Vivaldi/Opera(Chromium系)。Windows上のWSLは非対応
- **導入手順**:①ChromeウェブストアからClaude in Chrome拡張機能(v1.0.36以上)を入れる ②Claude Codeを`claude --chrome`で起動、または既存セッションで`/chrome`→「Enabled by default」。追加購入は不要だが、**直接契約のAnthropicプラン(Pro/Max/Team/Enterprise)であることが条件**(API経由や他社クラウド経由の利用では使えない)
- 権限:サイトごとにChrome拡張機能側の設定で許可を管理。操作の直前に「Claude in Chromeが◯◯したがっています」と確認が入り、「このセッション中はこのサイトで全部許可」も選べる
- 見た目を変える操作(クリック・入力・タブ操作)は毎回確認、読み取るだけの操作(ページを読む・スクショ)は確認無し(プランモード時)
- 注意点:常時有効にするとコンテキスト(AIが一度に覚えていられる情報量)を余分に消費するため、**使う時だけ`--chrome`を付ける運用が推奨**されている。録画機能(GIF)はログイン中のアカウント情報も映り込むため、外部共有前に要確認

**導入完了(2026-09-13・MacBook)**:拡張機能v1.0.93+Claude Code v2.1.270で接続テスト成功。つまずき:ChromeがDMG(インストール用ディスク)から直接起動されたままで未インストールだった→アプリケーションフォルダへ移して再起動で解決。VS Code拡張ではフラグ不要で、プロンプトに `@browser` と付けて頼む
